...
public class CoordinatorStockServlet extends HttpServlet {
    protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        HttpSession session = request.getSession(false);
        User user = (session != null) ? (User) session.getAttribute("user") : null;
        System.out.println("User role in GET: " + (user != null ? user.getRole() : "null"));
        if (session == null || user == null || !user.getRole().equals("HOSPITAL_COORDINATOR")) {
        ...
        try {
            ...
            e.printStackTrace();
            request.setAttribute("error", "Error retrieving stock");
            request.getRequestDispatcher("/jsp/hospital_coordinator/hospital_coordinator_stock.jsp").forward(request, response);
        }
    }
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        try {
            if ("/add".equals(action)) {
                ...
                response.sendRedirect(request.getContextPath() + "/coordinator/stock");
            }
